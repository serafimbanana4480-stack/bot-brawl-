...
class KalmanFilterPredictor:
    ...
    def update(self, measurement: tuple[float, float]):
        ...
        S = self.H @ self.covariance @ self.H.T + np.eye(2) * self.measurement_noise  # noqa: N806

        # Kalman gain
        K = self.covariance @ self.H.T @ np.linalg.inv(S)  # noqa: N806
...
class MovementPredictor:
    ...
    def _init_lstm(self, model_path: Path | None):
